...
class WildfireMetaLabeler:
    ...
    def build_features(
        ...
        N = len(prob_flat)
        ws_flat = np.full(N, wind_speed) if np.isscalar(wind_speed) else wind_speed.flatten()
        hum_flat = np.full(N, humidity) if np.isscalar(humidity) else humidity.flatten()
        temp_flat = np.full(N, temp) if np.isscalar(temp) else temp.flatten()
