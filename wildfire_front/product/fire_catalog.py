...
def product_action_catalog() -> list[dict[str, Any]]:
    ...
    raw = [
        ...
        },
        {
            ...
            "cmd": "python -m wildfire_front snapshot --work-dir DIR",
            "why": "Instantánea compartible (no despacho)",
        },
        {
            "id": "compare",
            "group": "Decisión",
            "title": "Qué cambió",
            "cmd": "python -m wildfire_front compare --work-dir DIR",
            "why": "Alerta local de cambio (no SMS)",
        },
        {
            "id": "replay",
            "group": "Decisión",
...
def new_fire_intake_steps() -> list[dict[str, Any]]:
    ...
    raw = [
        {
            "step": "1",
            "title": "Abre la carpeta del incendio",
            "detail": "Pulsa «Abrir carpeta» en la consola. Ahí se sueltan las fotos.",
            "cmd": "mkdir outputs\\incidents\\MI_IF\\inbox",
        },
        {
            "step": "2",
            "title": "Suelta las fotos térmicas (.tif)",
            "detail": "Un JPG del móvil no sirve. El nombre debe llevar fecha: 20260817_153000_frente.tif",
            "cmd": "python -m wildfire_front doctor --inbox outputs/incidents/MI_IF/inbox",
        },
        {
            "step": "3",
            "title": "Pulsa Procesar",
            "detail": "El sistema lee las fotos y actualiza la palabra grande. No es una orden.",
            "cmd": "python -m wildfire_front incident update --inbox outputs/incidents/MI_IF/inbox --work-dir outputs/incidents/MI_IF --force",
        },
        {
            "step": "4",
            "title": "Lee SEGUIR / ESPERAR / SE CALLA",
            "detail": "Callarse es correcto si faltan fotos. No lances medios por esta pantalla.",
            "cmd": "python -m wildfire_front decide --policy field_ops --work-dir outputs/incidents/MI_IF --explain",
        },
        {
            "step": "5",
            "title": "Si no ves el incendio, abre la consola",
            "detail": "Con --serve los botones hacen el trabajo. Sin serve, este comando abre la pantalla.",
            "cmd": "python -m wildfire_front app --work-dir outputs/incidents/MI_IF --open",
        },
        {
            "step": "6",
            "title": "Probar sin fotos reales",
            "detail": "Solo si no tienes .tif: incendio de juguete para ensayar la pantalla.",
