...
def load_gate_snapshot(repo: Path | None = None) -> dict[str, Any]:
    ...
    if plan_doc is not None:
        _rails_raw = plan_doc.get("rails")
        rails: dict[str, Any] = _rails_raw if isinstance(_rails_raw, dict) else {}
        _gates_raw = plan_doc.get("gates")
        g: dict[str, Any] = _gates_raw if isinstance(_gates_raw, dict) else {}
...
def format_decide_explain(
    ...
    *,
    ...
    metrics: dict[str, Any] = _metrics if isinstance(_metrics, dict) else {}
    policy_id = card.get("policy_id") or metrics.get("policy_id") or audit.get("policy_id") or "—"
    _snap = audit.get("policy_snapshot")
    snap: dict[str, Any] = _snap if isinstance(_snap, dict) else {}
