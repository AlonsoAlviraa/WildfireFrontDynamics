            ...
            writer.writerow(
                {
                    ...
                    "resolution_m": ""
                    if observation.resolution_m is None
                    else observation.resolution_m,
...
def write_arrival_csv(xx: np.ndarray, yy: np.ndarray, arrival: np.ndarray, output: Path) -> None:
    with output.open("w", newline="", encoding="utf-8") as handle:
        ...
        for x, y, time_s in zip(xx.ravel(), yy.ravel(), arrival.ravel(), strict=True):
            writer.writerow(
                [
                    x,
                    y,
    ...
    for index, observation in enumerate(observations):
        ...
        if observation.truth_components:
            for component in observation.truth_components:
                ...
                lines.append(
                    f'<polygon points="{truth}" fill="none" stroke="#7b8790" stroke-width="1" opacity=".45"/>'
                )
        for component in observation.components:
            observed = " ".join(f"{x:.1f},{y:.1f}" for x, y in map(project, component))
            lines.append(
                f'<polygon points="{observed}" fill="none" stroke="{color}" stroke-width="2" opacity=".9"/>'
            )
    lines.extend(
        [
...
def write_report(
    ...
    _speed_reasons = summary.get("speed_abstention_reasons")
    context = {
        ...
        "limitations": ", ".join(
            sorted({limit for item in observations for limit in item.limitations})
        )
        or "none",
        "speed_abstention_reasons": ", ".join(
            str(r) for r in (_speed_reasons if isinstance(_speed_reasons, list) else [])
        )
        or "none",
    }
    context_rows = "".join(
