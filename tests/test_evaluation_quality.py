...
class EvaluationQualityTests:
    def test_identical_front_has_zero_distance(self) -> None:
        ...
        assert pytest.approx(metrics["front_distance_mean"]) == 0.0
        assert pytest.approx(metrics["front_hausdorff"]) == 0.0
    ...
    def test_shifted_front_distance_is_metric(self) -> None:
        ...
        assert metrics["front_distance_mean"] > 0.0
        assert pytest.approx(metrics["front_hausdorff"], abs=0.05) == 2.0
    ...
    def test_observation_quality_detects_area_decrease(self) -> None:
        ...
        assert summary["observed_area_decrease_count"] == 1
        assert summary["interval_s_median"] == 60.0
    ...
    def test_turn_angles_identify_rectangle_corners(self) -> None:
        ...
        assert all(abs(value - 90.0) < 1e-06 for value in turns)
