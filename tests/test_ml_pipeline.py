...
pytestmark_torch = pytest.mark.skipif(torch is None, reason="PyTorch is not installed")
pytestmark_sklearn = pytest.mark.skipif(not HAS_SKLEARN, reason="scikit-learn is not installed")


@pytest.fixture
def images_dir() -> Path:
    return Path("data/candidates/semireal_controlled_001/images")


@pytest.fixture
def masks_dir() -> Path:
    return Path("data/candidates/semireal_controlled_001/masks")
...
@pytestmark_torch
def test_minimal_a3c_weights_load_clean() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert report["missing"] == [], f"fixture left missing keys: {report['missing']}"
...
@pytestmark_torch
def test_wildfire_dataset_loading_and_shapes(images_dir: Path, masks_dir: Path) -> None:
    ...
    assert len(dataset) > 0, "Dataset should have at least one valid sequence patch"
    ...
    assert torch.is_tensor(current_fire)
    assert torch.is_tensor(target_fire)


@pytestmark_torch
def test_fine_tuning_execution_one_epoch(images_dir: Path, masks_dir: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert output_weights.exists(), "Fine-tuned model checkpoint should be saved"


@pytestmark_torch
def test_cloud_train_build_parser() -> None:
    ...
    assert args.images == Path("img_path")
    assert args.masks == Path("mask_path")
    assert args.weights == Path("weights_path")
    assert args.output_weights == Path("out_path")


@pytestmark_torch
def test_cloud_train_execution_one_epoch_without_upload(images_dir: Path, masks_dir: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert output_weights.exists(), "Cloud weights checkpoint should be saved"


@pytestmark_torch
def test_focal_loss_penalizes_false_negatives_more() -> None:
    ...
    assert loss_fn.item() / max(loss_fp.item(), 1e-8) > 2.0, (
        "FN/FP loss ratio should be >= 2× with pos_weight=3.0"
    )


@pytestmark_torch
def test_focal_loss_gamma_zero_equals_weighted_bce() -> None:
    ...
    assert focal.item() == pytest.approx(bce.item(), abs=1e-5)


@pytestmark_torch
def test_smart_init_fusion_layers_preserves_features() -> None:
    ...
    assert "fusion_gate" in initialized
    assert "refine" in initialized
    assert "temporal_projection" in initialized
    ...
    assert torch.all(gate_bias < -3.0), "Gate bias should be < -3 for passthrough"
    ...
    for _name, module in model.refine.named_modules():
        if isinstance(module, nn.Conv2d):
            refine_conv = module
            break
    assert refine_conv is not None
    ...
    for c in range(min(refine_conv.out_channels, refine_conv.in_channels)):
        assert refine_conv.weight[c, c, center, center].item() == pytest.approx(1.0, abs=1e-4)


@pytestmark_torch
def test_load_pretrained_weights_filters_smart_init_from_warnings() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for layer in smart_layers:
            assert not any(k.startswith(layer + ".") for k in report["missing"]), (
                f"{layer} keys leaked into missing: {report['missing']}"
            )
            assert not any(k.startswith(layer + ".") for k in report["shape_mismatch"]), (
                f"{layer} keys leaked into shape_mismatch: {report['shape_mismatch']}"
            )
            assert not any(layer in msg for msg in warning_msgs), (
                f"{layer} appeared in a warning: {warning_msgs}"
            )

        # Smart-init must have run on all three layers.
        assert set(report["smart_init"]) == smart_layers


@pytestmark_torch
def test_npz_wildfire_dataset() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert seq.shape == (3, 17, 30, 30)
        assert curr.shape == (30, 30)
        assert target.shape == (30, 30)


@pytestmark_sklearn
def test_wildfire_meta_labeler() -> None:
    ...
    assert meta_labeler.is_trained
...
@pytestmark_sklearn
def test_meta_labeler_single_class_guard() -> None:
    ...
    assert meta_labeler.is_trained
    assert meta_labeler._single_class_label == 1
    ...
    assert meta_labeler._single_class_label == 0
    probs0 = meta_labeler.predict_probability(X)
    assert np.allclose(probs0, 0.0)


@pytestmark_sklearn
def test_meta_labeler_entropy_boundaries() -> None:
    ...
    assert ent[0] == pytest.approx(1.0, abs=1e-5)  # max entropy at p=0.5
    assert ent[1] < 1e-5  # near-zero at extremes
    assert ent[2] < 1e-5


@pytestmark_sklearn
def test_meta_labeler_entropy_symmetry_and_shape() -> None:
    ...
    assert h_p.shape == p.shape


@pytestmark_sklearn
def test_meta_labeler_predict_without_training_raises() -> None:
...
@pytestmark_sklearn
def test_meta_labeler_build_features_columns_and_2d() -> None:
    ...
    assert feats2d.shape == (16, 7)


@pytestmark_sklearn
def test_meta_labeler_save_load_roundtrip() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert loaded2._single_class_label == 1
        np.testing.assert_allclose(loaded2.predict_probability(X), 1.0)


@pytestmark_sklearn
def test_meta_labeler_determinism_via_random_state() -> None:
...
def test_preprocess_ndws_cli() -> None:
    ...
    assert "required" in (res.stderr.lower() + res.stdout.lower())
    ...
    # Accept TF missing, empty split, or missing Kaggle input dir (message variants).
    assert "tfrecord" in output or "tensorflow" in output or "no input directory" in output
