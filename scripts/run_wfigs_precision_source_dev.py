#!/usr/bin/env python3
"""Run the RCDA precision-tuned hybrid source on WFIGS DEV only."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"precision-source runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_precision_source_wrapper_v1",
        ...
        "reports": [
            {
                ...
                "config": {
                    ...
                    "tversky_alpha": float(payload.get("tversky_alpha", 0.7)),
                    "tversky_beta": float(payload.get("tversky_beta", 0.3)),
                    "tversky_gamma": float(payload.get("tversky_gamma", 0.75)),
                },
            }
        ],
    }
    with tempfile.TemporaryDirectory(prefix="wfd_precision_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("precision-source DEV-only invariant violated")
