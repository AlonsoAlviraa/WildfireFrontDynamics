#!/usr/bin/env python3
"""Run the preregistered far-background BCE WFIGS DEV experiment."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    ...
    if any(token in lowered for token in ("confirm", "test", "prospective")):
        raise ValueError(f"far-BCE runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_far_bce_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_far_bce_source_") as temporary:
        ...
        report = adapt_frozen_rcda_on_wfigs(
            ...
            adaptation=WFIGSAdaptConfig(
                ...
                far_background_bce_weight=0.05,
                far_background_min_distance_px=12.0,
                target_mode="hybrid",
                augment=True,
                source_seeds=(seed,),
            ),
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("far-BCE DEV-only invariant violated")
