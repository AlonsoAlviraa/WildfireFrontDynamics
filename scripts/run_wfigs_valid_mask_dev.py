#!/usr/bin/env python3
"""Adapt a residual source with WFIGS valid-data mask as a learned feature."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"valid-mask runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_valid_mask_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_valid_mask_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("valid-mask DEV-only invariant violated")
