#!/usr/bin/env python3
"""Adapt RCDA front geometry with the full first encoder block on DEV."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"front-geometry enc1 runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_front_geometry_enc1_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_front_geometry_enc1_source_") as temporary:
        ...
        report = adapt_frozen_rcda_on_wfigs(
            ...
            adaptation=WFIGSAdaptConfig(
                ...
                trainable_scope="decoder_plus_enc1",
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("front-geometry enc1 DEV-only invariant violated")
