#!/usr/bin/env python3
"""Adapt RCDA with robust per-tile EO residual features on DEV only."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"tile-standardized runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_tile_standardized_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_tile_standardized_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("tile-standardized DEV-only invariant violated")
