#!/usr/bin/env python3
"""Flatten spatial_v1_era5 fire dirs → holdout train → estrella_floor LOFO pack.

Weather provenance: era5_land_cds_v1 (real CDS ERA5-Land, IDW regrid).
Builds core-3 LOFO for WEATHER_LIFT vs prior spatial bridge board (~0.558).

Usage::

    $env:PYTHONPATH = "."
    python scripts/build_spatial_era5_lofo_pack.py
    python scripts/build_spatial_era5_lofo_pack.py --require-fires CARDOSO,LA_ESTRELLA_ACOM1,LA_ESTRELLA_ACOM2
"""
...
DEFAULT_SRC = ROOT / "artifacts" / "clm_ndws_patches" / "spatial_v1_era5"
DEFAULT_FLAT = ROOT / "artifacts" / "clm_ndws_patches" / "holdout_spatial_v1_era5"
DEFAULT_OUT = ROOT / "artifacts" / "clm_ndws_patches" / "lofo_mix_spatial_era5_v1"
DEFAULT_MANIFEST = ROOT / "outputs" / "ml_eval" / "lofo_mix_spatial_era5_v1_manifest.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    man["weather_provenance"] = "era5_land_cds_v1"
    man["not_era5"] = False
    man["feature_schema"] = "spatial_v1"
    man["sequence_shape"] = list(EXPECTED_SEQ)
    man["source_patch_counts"] = counts
    man["work_class"] = "feature_spatial_v1+weather_era5_land+estrella_floor"
    man["comparability"] = {
        "prior_spatial_bridge_mean": 0.5575550981918408,
        "prior_spatial_bridge_min": 0.48528418760127023,
        "weather_lift_threshold": 0.01,
        "note": (
            "Same residual-small + bridge adapted init; weather raster = ERA5-Land CDS. "
            "Compare vs W0 prior bridge mean 0.5576 (not vs Open-Meteo regression)."
        ),
    }
