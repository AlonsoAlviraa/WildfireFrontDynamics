#!/usr/bin/env python3
"""Adapt RCDA with front geometry and tile-standardized EO on DEV only."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"combined-feature runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_combined_features_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_combined_features_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("combined-feature DEV-only invariant violated")
