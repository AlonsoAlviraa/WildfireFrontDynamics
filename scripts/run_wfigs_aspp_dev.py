#!/usr/bin/env python3
"""Run an ASPP-U-Net RCDA source adaptation on WFIGS DEV only."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"ASPP runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    if payload.get("model_name") != "aspp_unet" or payload.get("target_mode") != "hybrid":
        raise ValueError("source checkpoint must be RCDA aspp_unet hybrid")
    seed = int(payload["seed"])
    source_summary = {
        "schema": "wfd_rcda_aspp_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_aspp_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("ASPP DEV-only invariant violated")
