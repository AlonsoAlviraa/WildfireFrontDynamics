#!/usr/bin/env python3
"""Run decoder-only WFIGS adaptation with TRAIN-fitted converted normalization."""
...
def _assert_dev_only(dataset_root: Path, normalization: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"domain-normalization runner accepts DEV only; refusing {dataset_root}")
    if normalization.name != "normalization_wfigs_converted_train_only.json":
        raise ValueError("runner requires the TRAIN-fitted converted WFIGS normalization")
...
def main() -> int:
    ...
    parser.add_argument("--wfigs-normalization", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--epochs", type=int, default=18)
    parser.add_argument("--patience", type=int, default=5)
    args = parser.parse_args()
    _assert_dev_only(args.wfigs_dev_root, args.wfigs_normalization)
    ...
    source_summary = {
        "schema": "wfd_rcda_domain_norm_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_domain_norm_source_") as temporary:
        ...
        report = adapt_frozen_rcda_on_wfigs(
            ...
            rcda_normalization_path=args.wfigs_normalization,
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("domain-normalization DEV-only invariant violated")
