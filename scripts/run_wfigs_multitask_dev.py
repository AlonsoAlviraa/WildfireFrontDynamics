#!/usr/bin/env python3
"""Adapt an RCDA multitask source on WFIGS expansion DEV only."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    ...
    if any(token in lowered for token in ("confirm", "test", "prospective")):
        raise ValueError(f"multitask runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    if payload.get("model_name") != "resunet_multitask" or payload.get("target_mode") != "multitask":
        raise ValueError("source checkpoint must be RCDA resunet_multitask")
    seed = int(payload["seed"])
    source_summary = {
        "schema": "wfd_rcda_multitask_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_multitask_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("multitask DEV-only invariant violated")
