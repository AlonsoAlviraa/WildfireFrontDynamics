#!/usr/bin/env python3
"""Run one preregistered negative-background BCE WFIGS DEV experiment."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    ...
    if any(token in lowered for token in ("confirm", "test", "prospective")):
        raise ValueError(f"negative-BCE runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_negative_bce_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_negative_bce_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("negative-BCE DEV-only invariant violated")
