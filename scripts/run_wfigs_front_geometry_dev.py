#!/usr/bin/env python3
"""Adapt an RCDA source with explicit front geometry features on WFIGS DEV."""
...
def _assert_dev_only(dataset_root: Path) -> None:
    if any(token in dataset_root.name.lower() for token in ("confirm", "test", "prospective")):
        raise ValueError(f"front-geometry runner accepts DEV only; refusing {dataset_root}")
...
def main() -> int:
    ...
    source_summary = {
        "schema": "wfd_rcda_front_geometry_source_wrapper_v1",
    ...
    with tempfile.TemporaryDirectory(prefix="wfd_front_geometry_source_") as temporary:
    ...
    if report["wfigs_test_loaded"] or report["test_used_for_selection"]:
        raise RuntimeError("front-geometry DEV-only invariant violated")
