#!/usr/bin/env python3
"""U-Net training pipeline v18 — Residual Delta Edition.

Predicts fire as logit(prev_fire) + delta, targeting improvement over the
copy baseline (PrevFireMask → FireMask, IoU ~0.79).
...
# --------------------------------------------------------------------------- #
parser = argparse.ArgumentParser(description="Wildfire Residual U-Net v18 training")
...
parser.add_argument("--version-tag", type=str, default="v18",
                    help="Experiment version label for training_summary.json.")
args, _unknown = parser.parse_known_args()

print("=" * 70)
print(f"WILDFIRE RESIDUAL U-NET TRAINING v18 — DELTA OVER COPY BASELINE")
print("=" * 70)
...
# --------------------------------------------------------------------------- #
# 1. Inline Kaggle helpers (companion .py files are NOT uploaded to Kaggle)
# --------------------------------------------------------------------------- #
...
def _install_pytorch_p100_compat():
    try:
...
def _validate_dataset_sizes(train_n, val_n, test_n):
...
def _select_device():
    """Verify CUDA kernels actually run (P100 sm_60 fails on PyTorch 2.10+)."""
    ...
    try:
        ...
        print(f"[WARN] CUDA probe failed on {name} sm_{cap[0]}{cap[1]}: {exc}")
        print("[WARN] P100 + PyTorch 2.10 incompatible — falling back to CPU.")
        return torch.device("cpu"), False
...
if device.type == "cpu":
    ...
    print(f"CPU fallback: epochs={args.epochs}, batch_size={args.batch_size}")
else:
    print(f"GPU training: epochs={args.epochs}, batch_size={args.batch_size}")
...
try:
    from models.unet_model import (
        WildfireUNet, WildfireUNetSmall, ResidualWildfireUNetSmall, count_parameters,
...
if not MODEL_FROM_REPO:
    ...
    class ResidualWildfireUNetSmall(nn.Module):
        def __init__(self, in_channels=12, bilinear=True, norm="group", se_attention=False):
            super().__init__()
            self.backbone = WildfireUNetSmall(in_channels, 1, bilinear, norm, se_attention)

        def forward(self, x, prev_fire):
            if prev_fire.dim() == 3:
                prev_fire = prev_fire.unsqueeze(1)
            copy_logits = torch.logit(prev_fire.float().clamp(1e-4, 1.0 - 1e-4))
...
_validate_dataset_sizes(len(train_dataset), len(val_dataset), len(test_dataset))
...
model = ResidualWildfireUNetSmall(
    in_channels=in_channels, bilinear=True,
    norm=args.norm, se_attention=args.se_attention,
)
model.to(device)
model_cls = ResidualWildfireUNetSmall
...
def evaluate_model(model, loader, device, thresholds=(0.3, 0.4, 0.5)):
    ...
    copy_metrics = {t: [] for t in thresholds}
    fire_pixel_ratios = []
        ...
        for i in range(probs.shape[0]):
            ...
            for t in thresholds:
                try:
                    ...
                    prev_bin = np.where(prev_np < 0, 0.0, prev_np)
                    prev_bin = (prev_bin >= t).astype(np.float64)
                    m = compute_segmentation_metrics(pred_np, tgt_np, threshold=t)
                    cm = compute_segmentation_metrics(prev_bin, tgt_np, threshold=t)
                    thresh_metrics[t].append(m)
                    copy_metrics[t].append(cm)
                    continue
                sample = evaluate_sample(pred_np, prev_np, tgt_np, threshold=t)
                thresh_metrics[t].append(sample["model_full"])
                copy_metrics[t].append(sample["copy_full"])
    ...
    for t in thresholds:
        ...
        copy_seg = aggregate_segmentation_metrics(copy_metrics[t])
        results[f"copy_baseline_thresh_{t}"] = copy_seg

    if fire_pixel_ratios:
        results["mean_fire_pixel_ratio"] = float(np.mean(fire_pixel_ratios))

    primary = results.get("thresh_0.5", {})
    copy_primary = results.get("copy_baseline_thresh_0.5", {})
    results["improvement_vs_copy_iou"] = (
        primary.get("micro_iou", 0.0) - copy_primary.get("micro_iou", 0.0)
    )
...
log_msg(f"\n--- Residual U-Net v18 started at {time.strftime('%Y-%m-%d %H:%M:%S')} ---")
...
copy_iou = test_results.get("copy_baseline_thresh_0.5", {}).get("micro_iou", 0.0)
model_iou = test_results.get("thresh_0.5", {}).get("micro_iou", 0.0)
print(f"\n  Copy baseline IoU@0.5: {copy_iou:.4f}")
print(f"  Model IoU@0.5: {model_iou:.4f}")
print(f"  Improvement vs copy: {model_iou - copy_iou:+.4f}")
print("\n=== RESIDUAL U-NET v18 COMPLETED ===")