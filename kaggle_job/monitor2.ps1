...
while ($true) {
    $status = & kaggle kernels status alonsoalviraaaa/wildfire-front-training 2>&1 | Out-String
    ...
    if ($status -match "COMPLETE|ERROR|CANCEL") {
        ...
        & kaggle kernels output alonsoalviraaaa/wildfire-front-training -p $outdir 2>&1 | Out-File -Append -FilePath $log
