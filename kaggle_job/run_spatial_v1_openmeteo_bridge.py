#!/usr/bin/env python3
"""Kaggle: spatial_v1 LOFO with Open-Meteo weather + adapted multi_if→15ch init.

Data: alonsoalviraaaa/wfd-lofo-spatial-openmeteo-v1
Init: alonsoalviraaaa/wfd-spatial-bridge-init-weights (or multi_if adapt on-box)

Comparability:
  * Prior spatial bridge (DEM-lapse / previous weather) mean≈0.5576 min≈0.4853
  * WEATHER_LIFT if Δmean ≥ 0.01 (open_meteo vs that board) — same folds/recipe
  * NOT sealed recipe_t1 · NOT ERA5 (provenance open_meteo_archive_interp_v1)
...
def _maybe_zip(zpath: Path) -> Path | None:
    dest = Path("/tmp/lofo_spatial_openmeteo")
    if dest.exists():
        shutil.rmtree(dest)
...
def _data_root() -> Path:
    hard = [
        Path("/kaggle/input/wfd-lofo-spatial-openmeteo-v1"),
        Path("/kaggle/input/datasets/alonsoalviraaaa/wfd-lofo-spatial-openmeteo-v1"),
        Path("/kaggle/input/wfd-lofo-spatial-openmeteo-v1/lofo_mix_spatial_openmeteo_v1"),
        Path(
            "/kaggle/input/datasets/alonsoalviraaaa/wfd-lofo-spatial-openmeteo-v1/"
            "lofo_mix_spatial_openmeteo_v1"
        ),
    ]
    for c in hard:
    ...
    if Path("/kaggle/input").is_dir():
        ...
        for z in Path("/kaggle/input").rglob("*.zip"):
            ...
            if got is not None:
                return got
    raise FileNotFoundError("spatial openmeteo LOFO pack missing")
...
def main() -> int:
    print("=" * 70, flush=True)
    print("SPATIAL_V1 OPEN-METEO + BRIDGE ADAPTED INIT LOFO", flush=True)
    ...
    print("weather_provenance=open_meteo_archive_interp_v1 NOT_ERA5", flush=True)
    print("=" * 70, flush=True)
    ...
    for held in CORE3:
        ...
        cfg = UNetTrainConfig(
            ...
            output_dir=str(out),
            version_tag=f"spatial_openmeteo_{held}",
        ...
        row: dict = {
            "held": held,
            "status": "ok",
            "experiment_id": "E_spatial_v1_openmeteo_bridge",
            "feature_schema": "spatial_v1",
            "work_class": "feature_spatial_v1+weather_openmeteo+bridge_init",
    ...
    board = {
        "schema": "wfd_kaggle_spatial_v1_openmeteo_bridge_v1",
        ...
        "WEATHER_NULL": weather_null and not weather_lift,
        ...
        "note": (
            "Open-Meteo gridded weather re-emit + bridge adapted init. "
            "WEATHER_LIFT if mean − prior_bridge_mean ≥ 0.01. Not ERA5-Land."
        ),
    }
    (out_root / "spatial_v1_openmeteo_bridge_board.json").write_text(
