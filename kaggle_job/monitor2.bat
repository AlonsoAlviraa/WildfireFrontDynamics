...
:LOOP
for /f "tokens=2 delims=:," %%a in ('kaggle kernels status alonsoalvira/wildfire-front-training 2^>^&1 ^| findstr "status"') do set STATUS=%%a
...
if %errorlevel% equ 0 (
    ...
    kaggle kernels output alonsoalvira/wildfire-front-training -p "%OUTDIR%" >> "%LOG%" 2>&1
