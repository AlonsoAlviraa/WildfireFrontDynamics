...
Data: alonsoalviraaaa/wfd-lofo-spatial-era5-mf-v1 (core3 + hellin/braz train fill)
...
def _maybe_zip(zpath: Path) -> Path | None:
    dest = Path("/tmp/lofo_spatial_era5_mf")
    if dest.exists():
        shutil.rmtree(dest)
...
def _data_root() -> Path:
    hard = [
        Path("/kaggle/input/wfd-lofo-spatial-era5-mf-v1"),
        Path("/kaggle/input/datasets/alonsoalviraaaa/wfd-lofo-spatial-era5-mf-v1"),
        Path("/kaggle/input/wfd-lofo-spatial-era5-mf-v1/lofo_mix_spatial_era5_mf_v1"),
        Path(
            "/kaggle/input/datasets/alonsoalviraaaa/wfd-lofo-spatial-era5-mf-v1/"
            "lofo_mix_spatial_era5_mf_v1"
        ),
    ]
    for c in hard:
    ...
    if Path("/kaggle/input").is_dir():
        ...
        for z in Path("/kaggle/input").rglob("*.zip"):
            ...
            if got is not None:
                return got
    raise FileNotFoundError("spatial ERA5 multi-fire LOFO pack missing")
...
def main() -> int:
    print("=" * 70, flush=True)
    print("SPATIAL_V1 ERA5 MF (hellin+braz) + BRIDGE ADAPTED INIT LOFO", flush=True)
    ...
    for held in CORE3:
        ...
        cfg = UNetTrainConfig(
            ...
            output_dir=str(out),
            version_tag=f"spatial_era5_mf_{held}",
        ...
        row: dict = {
            "held": held,
            "status": "ok",
            "experiment_id": "E_spatial_v1_era5_mf_bridge",
            "feature_schema": "spatial_v1",
            "work_class": "feature_spatial_v1+weather_era5_land+multifire+bridge_init",
    ...
    board = {
        "schema": "wfd_kaggle_spatial_v1_era5_mf_bridge_v1",
        ...
        "GOAL_DELTA_MEAN": 0.05,
        "GOAL_MET_PLUS_0_05": bool(delta_mean is not None and delta_mean >= 0.05),
        ...
        "note": (
            "ERA5-Land multi-fire (hellin+braz train fill) + bridge adapted init. "
            "Compare vs W0 prior bridge mean 0.5576. GOAL +0.05 absolute mean IoU."
        ),
    }
    (out_root / "spatial_v1_era5_mf_bridge_board.json").write_text(
